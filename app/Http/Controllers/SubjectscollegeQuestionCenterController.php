...
class SubjectscollegeQuestionCenterController extends Controller
{
  ...
  elseif(Auth::user() &&Auth::user()->is_student == 5 && Auth::user()->category_id == 2){
    ...
    ->with('colleges',College::all())->with('divisions',$divisions)->
     with('sections',$sections)->with('subcolleges',$subcolleges)->with('users',$users)
...
$duration =  $file['playtime_seconds'];
  ...
  public function subjectscollegequestionscenter(){
    if(Auth::user() && Auth::user()->isAdmin == 'admin'){
      ...
      $questions = SubjectscollegePart::where('center_id',Auth::user()->id)->get();
        ...
        $sections = Section::all();
        $subcolleges = SubjectsCollege::all();
    return view("dashboard.subjectscollegequestions.subjectscollegequestionscenter.index")->with("questions",$questions)->with('colleges',College::all())->with('divisions',$divisions)->
  ...
  elseif(Auth::user() &&Auth::user()->is_student == 5 && Auth::user()->category_id == 2){
    ...
    return view("dashboard.subjectscollegequestions.subjectscollegequestionscenter.edit")->with('part',$part)->with('colleges',College::all())->with('divisions',$divisions)->
     with('sections',$sections)->with('subcolleges',$subcolleges)->with('users',$users)
     ->with('universities',University::all());
