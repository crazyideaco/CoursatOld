                ...
                <div class="setting">
                    <div class="container">
                        <div class="row def">
                            ...
                            <h5>تعديل قسم فرعى </h5>
                        </div>
                            <form method="post" action="{{route('updatesub',$sub->id)}}" enctype="multipart/form-data">
                        	@csrf
                        ...
                        <div class="info">
                         ...
                         <label>اسم القسم الفرعى بالعربى</label>
                                    <input class="form-control" type="text" name="name_ar" value="{{$sub->name_ar}}">
                                     @error('name_ar')
                         ...
                         <label>اسم القسم الفرعى بالانجليزيه</label>
                                    <input class="form-control" type="text" name="name_en" value="{{$sub->name_en}}">
                                     @error('name_en')
                                    <p style="color:red;">{{$message}}</p>
                                    @enderror
                                </div>
                                <div class="form-group col-lg-4 col-md-6 col-12">
                                    <label>اختر القسم العام</label>
                                    ...
                                    @if($sub->general_id == $general->id) selected @endif>{{$general->name_ar}}</option>
                                        @endforeach
                                    </select>
                                     @error('general_id')
                                    <p style="color:red;">{{$message}}</p>
