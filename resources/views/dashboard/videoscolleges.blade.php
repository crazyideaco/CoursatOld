            ...
            <div class="setting all-products typs">
                <div class="container">
                    <div class="row def">
                        ...
                        <h5 class="mr-4">{{ $lesson->typescollege ? $lesson->typescollege->doctor->name ?? '' : '' }}
                        </h5>


                        <h5 class="mr-4">
                            {{ $lesson->typescollege ? $lesson->type->center->name ?? 'المنصه العامه' : '' }}
                        </h5>
                    ...
                    <div class="products-search typs1">
                        <div class="row">
                            <div class="col-3">
                                <button class="btn">
                                    <a href="{{ route('addvideoscollege', $id) }}"> <span><i
                            ...
                            <div class="col-5">
                                <button class="btn" style="width:100% !important;">
                                    <a href="{{ route('addvideoscollegespecial', $id) }}"> <span><i
                                                class="fas fa-plus-circle"></i></span>
                ...
                <div class="pt-5">
                    <div class="row">
                     ...
                     <img src="{{ url('uploads/' . $video->image) }}" style="width:120px;height:120px">
                                        ...
                                        <td scope="col" class="text-center">{{ $video->subjectscollege['name_ar'] }}
                                        </td>

                                        <td scope="col" class="text-center">{{ $video->user->name }}</td>
                                        <td scope="col" class="text-center">{{ $video->order_number }}</td>
                                        <td class="text-center">
                                            <a href="{{ route('editvideoscollege', $video->id) }}"> <img
                                                    src="{{ asset('images/pen.svg') }}" id="pen"
                                                    style="cursor: pointer"></a>
                                            <span class="btn btn-success btn-sm" id="btn{{ $video->id }}"
                                                onclick="activevideoco({{ $video->id }})">
                                                @if ($video->active == 1)
                                                    الغاء التفعيل
                                                @else
                                                    تفعيل
                              ...
                              الامتحانات
                                            ...
                                            @if (auth()->user()->hasPermission('videoscolleges-delete'))
                                                <img src="{{ asset('images/trash.svg') }}" id="trash"
                                                    onclick="deletevideoscollege('{{ $video->id }}')"
