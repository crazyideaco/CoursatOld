                            ...
                            <div class="row">
                                <div class="col-lg-3 col-md-6 col-12">
                                    <button class="btn w-100 mx-auto" >
                                      <a href="{{route('addsubquestioncenters')}}">  <span><i class="fas fa-plus-circle"></i></span>
                                        اضافة   سؤال 
                                        </a>
                                    </button>
   ...
   <thead>
        ...
        <tbody>
                    ...
                    <td scope="row" class="text-center">{{$question->name}}</td>
                               <td class="text-center">{{$question->general->name_ar}}</td>
                         <td class="text-center">{{$question->sub->name_ar}}</td>
                        <td class="text-center">
                          <a href="{{route('editsubquestioncenters',$question->id)}}" > <img src="{{asset('images/pen.svg')}}" id="pen" 
                         style="cursor: pointer"></a>
                          @if(auth()->user()->hasPermission("subquestioncenterss-delete"))
...
$(`#btn${id}`).html('الغاء التفعيل');
 ...
 $.ajaxSetup({
  ...
  if (result.isConfirmed) {
    $.ajax({
       type:"get",
       url: `../deletesubquestioncenters/${id}`,
   //    contentType: "application/json; charset=utf-8",
       dataType: "Json",
       success: function(result){
